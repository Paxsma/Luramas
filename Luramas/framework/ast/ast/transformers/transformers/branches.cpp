...
void luramas::ast::transformers::branches::process_nested_logical_conditions(std::shared_ptr<luramas::ast::ast> &ast, const std::uintptr_t begin, const std::uintptr_t end, const std::vector<std::uintptr_t> dead /* Always opposite and when hit. */, const std::int16_t logical_operation_target /* Used for ignoring ands/ors. */, const bool check_loops /* Checks jumps too see if they lead too loop by expr and preform opposite. */, const bool last_include /* Includes last compare as or. */, const bool logical_operation /* ??? */) {
      ...
      std::unordered_map<std::uintptr_t /* Jmp loc */, std::size_t /* Count */> jmp_xrefs;
      ...
      for (const auto &node : range) {
            ...
            if (node->lex->kind == luramas::il::lexer::inst_kinds::branch_condition || node->lex->kind == luramas::il::lexer::inst_kinds::branch) {
                  ...
                  const auto jmp_location = node->lex->disassembly->ref->addr;

                  /* Add hit */
                  if (jmp_xrefs.find(jmp_location) == jmp_xrefs.end()) {
                        jmp_xrefs.insert(std::make_pair(jmp_location, 0u));
                  } else {
                        ++jmp_xrefs[jmp_location];
                  }

                  /* Hit dead? */
                  if (std::find(dead.begin(), dead.end(), jmp_location) != dead.end()) {
                        ...
                        if (!scopes.empty() && scopes.back().second) {
                              ...
                              node->add_elem<luramas::ast::element_kinds::condition_end_scope>(scopes.back().second);
                        }

                        /* Not the last branch. */
                        if (conditions.back() != node) {
                              debug_success("Condition isnt last adding and expr.");
                              node->add_elem<luramas::ast::element_kinds::condition_or>();
                        }
                        ...
                        if (!logical_operation) {
                              ...
                              if (node->has_elem(luramas::ast::element_kinds::condition_or)) {
                                    node->replace_next<luramas::ast::element_kinds::condition_or, luramas::ast::element_kinds::condition_and>(node->get_elem<luramas::ast::element_kinds::condition_or>().second);
                              }
                        ...
                        const auto prev = ast->body->visit_previous_addr(jmp_location);
                        if (check_loops && prev != nullptr && stat_jump_break_out(prev)) {
                        ...
                        if (scopes.empty()) {
                              scopes.push_back(std::make_pair(jmp_location, 0u));
                        }
                        ...
                        if (node->has_elem(luramas::ast::element_kinds::condition_emit_next)) {
                              ...
                              if (logical_operation_target == -1) {
                                    continue;
                              } else if (logical_operation_target != -1 && ast->body->visit_next(node)->lex->operand_kind<luramas::il::lexer::operand_kinds::dest>().front()->dis.reg != logical_operation_target) {
                                    continue;
                              }
                        }

                        /* Jmp exceeds dead values. */
                        if (jmp_location > over_target) {
                              ...
                              if (scopes.back().first == (node->address + node->lex->disassembly->len)) {
                                    ...
                                    if (!scopes.empty()) {
                                          ...
                                          if (jmp_xrefs[jmp_location]) {
                                                debug_success("Adding or expr.");
                                                node->add_elem<luramas::ast::element_kinds::condition_or>();
                                          } else {
                                                debug_success("Adding and expr.");
                                                node->add_elem<luramas::ast::element_kinds::condition_and>();
                                          }
                                    }
                                    ...
                                    /* New and sub scope */
                                    if (scopes.back().first != jmp_location && scopes.back().first > jmp_location) {
                                          ...
                                          node->add_elem<luramas::ast::element_kinds::condition_start_scope>();
                                          scopes.push_back(std::make_pair(jmp_location, 1u /* Starts off with one could inc. */));
                                          node->branches.opposite = true;

                                    } else {

                                          /* Jmps too end. */
                                          if (jmp_location == end || jmp_location == (end + ast->body->visit_addr(end)->lex->disassembly->len)) {
                                                ...
                                                if (scopes.back().first != jmp_location && has_val(jmp_location)) {
...
void luramas::ast::transformers::branches::set_branch_safety_analyzer(std::shared_ptr<luramas::ast::ast> &ast) {

      std::size_t idx = 0u;
      std::size_t routine = 0u;

      const auto conditions = std::get<std::vector<std::shared_ptr<luramas::ast::node>>>(ast->body->visit_inst_kinds<luramas::il::lexer::inst_kinds::compare>(true));
      ...
      auto condition = conditions[idx++]->lex->operand_kind<luramas::il::lexer::operand_kinds::compare>();
      ...
      for (auto &node : all) {
            ...
            node->lex->operand_kind_callback<luramas::il::lexer::operand_kinds::source>(check_usage);
            node->lex->operand_kind_callback<luramas::il::lexer::operand_kinds::reg>(check_usage);
            ...
            if (!routine && node->lex->has_operand_kind<luramas::il::lexer::operand_kinds::dest>()) {
                  ...
                  /* If target exist i.e. cmp r1, r2 check to see if dest compares equals to it. */
            ...
            if (node->lex->kind == luramas::il::lexer::inst_kinds::branch_condition) {
                  ...
                  start_node->add_elem<luramas::ast::element_kinds::desc_conditional_routine_start>();

                  node->add_elem<luramas::ast::element_kinds::desc_conditional_routine_end>();

                  const auto range_br = ast->body->visit_range(start_node->address, node->address);
                  for (const auto &i : range_br) {
                        i->add_elem<luramas::ast::element_kinds::desc_conditional_routine>();
                  }

                  /* Set next */
                  if (conditions.size() != idx) {
                        condition = conditions[idx++]->lex->operand_kind<luramas::il::lexer::operand_kinds::compare>();
...
void luramas::ast::transformers::branches::set_branch_statements(std::shared_ptr<luramas::ast::ast> &ast) {
      ...
      for (const auto &i : routines) {
            ...
            if (i.second->lex->kind == luramas::il::lexer::inst_kinds::branch_condition && !i.first->has_elem(luramas::ast::element_kinds::condition_concat_start) && !i.first->has_elem(luramas::ast::element_kinds::condition_concat_end)) {

                  /* If or elseif/else */
                  const auto ref_addr = i.second->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr;
                  ...
                  /* If statement */
                  concat_start->add_elem<luramas::ast::element_kinds::condition_concat_start>();
                  concat_end->add_elem<luramas::ast::element_kinds::condition_concat_end>();
                  concat_end->add_elem<luramas::ast::element_kinds::stat_if>();
                  debug_success("If statement set on %s", concat_end->str().c_str());

                  /* See if jump is greater then 1. */
                  luramas::ast::transformers::branches::process_nested_logical_conditions(ast, concat_start->address, concat_end->address, {ref_addr}, -1, true);
                  ...
                  /* Loop for jumps. */
                  while (prev_jmp->lex->disassembly->op == luramas::il::arch::opcodes::OP_JUMP /* Jump forward else */ && !prev_jmp->has_elem(luramas::ast::element_kinds::stat_loop_break)) {
                        ...
                        if (!ref_addr_node->has_elem(luramas::ast::element_kinds::condition_logical_start) || !ref_addr_node->has_elem(luramas::ast::element_kinds::condition_logical_end)) {
                              ...
                              if (next_jmp == nullptr || next_jmp->address > prev_jmp->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr ||
                                  !next_jmp->has_elem(luramas::ast::element_kinds::condition_logical_start) || !next_jmp->has_elem(luramas::ast::element_kinds::condition_logical_end) ||
                                  !ast->body->visit_next(prev_jmp)->has_elem(luramas::ast::element_kinds::condition_logical_start)) {

                                    debug_line("Next jump from jump target doesn't exist, out of scope, doesn't have any conditions, etc.");

                                    routine_end = ast->body->visit_addr(prev_jmp_const->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr);
                                    prev_jmp->add_elem<luramas::ast::element_kinds::jump_else>();
                                    ref_addr_node->add_elem<luramas::ast::element_kinds::stat_else>();
                        ...
                        const auto ref_addr_cl_routine_end_jmp = ref_addr_cl_routine_end->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr;

                        /* Already analyzed */
                        if (ref_addr_node->has_elem(luramas::ast::element_kinds::condition_concat_start)) {
                              debug_warning("Jump target already has been analyzed on %s", ref_addr_node->str().c_str());
                              break;
                        }

                        /* Condition is different else if */
                        if (prev_jmp->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr != prev_jmp_const->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr) {
                              /* Set end */
                        ...
                        /* No elseif */
                        if (!ref_addr_cl_routine_end->has_elem(luramas::ast::element_kinds::stat_elseif)) {

                              debug_line("Jump location next conditional logical routine doesn't have elseif on %s", ref_addr_cl_routine_end->str().c_str());

                              /* elseif statement */
                              ref_addr_node->add_elem<luramas::ast::element_kinds::condition_concat_start>();
                              ref_addr_cl_routine_end->add_elem<luramas::ast::element_kinds::condition_concat_end>();
                              ref_addr_cl_routine_end->add_elem<luramas::ast::element_kinds::stat_elseif>();
                              prev_jmp->add_elem<luramas::ast::element_kinds::jump_elseif>();

                              debug_success("Set elseif statement on %s", prev_jmp->str().c_str());

                              /* Set data */
                              luramas::ast::transformers::branches::process_nested_logical_conditions(ast, ref_addr_node->address, ref_addr_cl_routine_end->address, {ref_addr_cl_routine_end_jmp}, -1, true);
                        ...
                        /* Set next */
                        ref_addr_node = ast->body->visit_addr(ref_addr_cl_routine_end->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr);
                        prev_jmp = ast->body->visit_previous_addr(ref_addr_node->address);
                  }

                  /* Fix routine_end based on jump. */
                  if (scope_end != nullptr && scope_end->lex->disassembly->op == luramas::il::arch::opcodes::OP_JUMP && ref_addr == scope_end->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr) {
                  ...
                  routine_end->add_elem<luramas::ast::element_kinds::stat_scope_end>(1u, (concat_end->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr > 1u) ? luramas::ast::element::front : luramas::ast::element::back /* Empty expression? */);
                  debug_success("Set end expr on %s", routine_end->str().c_str());

                  /* Loop/while/repeat/generic */
                  const auto prev = ast->body->visit_previous_addr(concat_end->lex->operand_kind<luramas::il::lexer::operand_kinds::jmpaddr>().front()->ref_addr /* Take jump */);
                  if (stat_jump_break_out(prev)) {

                        debug_line("Previous breaks out of loop routine on %s", prev->str().c_str());

                        /* Append break */
                        concat_end->add_elem<luramas::ast::element_kinds::stat_jump_break>();

                        /* Remove scope end */
                        routine_end->remove_elem<luramas::ast::element_kinds::stat_scope_end>();
...
void luramas::ast::transformers::branches::set_jumpout_elements(std::shared_ptr<luramas::ast::ast> &ast) {
      ...
      for (const auto &i : compares) {
            ...
            if (scopes.empty()) {
                  ...
                  if (current_jump > scopes.back() /* Hit?? */ && !i->has_elem(luramas::ast::element_kinds::conditional_break)) {
                        i->add_elem<luramas::ast::element_kinds::desc_conditional_jumpout>();
                  } else {
...
std::shared_ptr<luramas::ast::node> luramas::ast::transformers::branches::find_jump_out(std::shared_ptr<luramas::ast::ast> &ast, const std::shared_ptr<luramas::ast::node> start, const std::shared_ptr<luramas::ast::node> end) {

      std::shared_ptr<luramas::ast::node> result = nullptr;
      bool changed = false; /* Result has been changed? */

      do {
            changed = false;

            const auto range = ast->body->visit_range(start->address, (result == nullptr) ? end->address : result->address);
            for (const auto &node : range) {

                  if (node->lex->kind == luramas::il::lexer::inst_kinds::branch_condition && node->lex->disassembly->ref->addr > end->address) {

                        if (result == nullptr || result->lex->disassembly->ref->addr < node->lex->disassembly->ref->addr) {
                              result = ast->body->visit_addr(node->lex->disassembly->ref->addr);
                              changed = true;
                        }
                  }
            }
