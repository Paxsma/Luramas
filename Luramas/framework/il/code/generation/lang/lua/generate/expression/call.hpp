                  ...
                  namespace call {
                        ...
                        __inline void emit_call(luramas::ir::data::refrence_string &buffer, const luramas::ir::data::refrence_string &call, std::vector<luramas::ir::data::refrence_string> &vect, const std::shared_ptr<luramas::ir::data::format::format> &format) {
                              ...
                              for (auto i = 0u; i < count; ++i) {
                                    ...
                                    /* Add split. */
                                    if ((i + 1u) != count) {
                                          args += format->spacing.format_parameter_seperator(lua_langkeyword_call_seperator);
                                    }
                              }

                              /* ( ?? ) */
                              args.push_front(format->spacing.format_call_parameter_open(lua_langkeyword_call_parenthesis_open));
                              args += format->spacing.format_call_parameter_close(lua_langkeyword_call_parenthesis_close);
