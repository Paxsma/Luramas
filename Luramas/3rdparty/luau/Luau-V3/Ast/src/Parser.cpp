...
#include "Luau-V3/Ast/include/Luau/Parser.h"

#include "Luau-V3/Ast/include/Luau/TimeTrace.h"
...
namespace Luau {
      ...
      std::pair<AstExprFunction *, AstLocal *> Parser::parseFunctionBody(
          bool hasself, const Lexeme &matchFunction, const AstName &debugname, const Name *localName) {
            ...
            return {allocator.alloc<AstExprFunction>(Location(start, end), generics, genericPacks, self, vars, vararg, varargLocation, body, functionStack.size(), debugname, typelist, varargAnnotation, hasEnd, argLocation),
                funLocal};
      }
      ...
      AstType *Parser::parseFunctionTypeAnnotationTail(const Lexeme &begin, AstArray<AstGenericType> generics, AstArray<AstGenericTypePack> genericPacks, AstArray<AstType *> &params, AstArray<std::optional<AstArgumentName>> &paramNames, AstTypePack *varargAnnotation)
      ...
      AstType *Parser::parseTypeAnnotation(TempVector<AstType *> &parts, const Location &begin) {
            ...
            if (isUnion && isIntersection) {
                  return reportTypeAnnotationError(Location(begin, parts.back()->location), copy(parts), /*isMissing*/ false, "Mixing union and intersection types is not allowed; consider wrapping in parentheses.");
            }
      ...
      AstTypeOrPack Parser::parseSimpleTypeAnnotation(bool allowPack) {
            ...
            if (lexer.current().type == Lexeme::ReservedNil) {
                  ...
                  return {reportTypeAnnotationError(start, {}, /*isMissing*/ false, "Using 'function' as a type annotation is not supported, consider replacing with a function type annotation e.g. '(...any) -> "
                                                                                    "...any'"),
                      {}};
            } else {
      ...
      LUAU_NOINLINE AstExpr *Parser::reportFunctionArgsError(AstExpr *func, bool self) {
            if (self && lexer.current().location.begin.line != func->location.end.line) {
                  ...
                  return reportExprError(Location(func->location.begin, lexer.current().location.begin), copy({func}), "Expected '(', '{' or <string> when parsing function call, got %s", lexer.current().toString().c_str());
            }
      }
      ...
      LUAU_NOINLINE void Parser::expectMatchAndConsumeFail(Lexeme::Type type, const MatchLexeme &begin, const char *extra) {
            ...
            if (lexer.current().location.begin.line == begin.position.line)
                  report(lexer.current().location, "Expected %s (to close %s at column %d), got %s%s", typeString.c_str(), matchString.c_str(), begin.position.column + 1, lexer.current().toString().c_str(), extra ? extra : "");
            else
                  report(lexer.current().location, "Expected %s (to close %s at line %d), got %s%s", typeString.c_str(), matchString.c_str(), begin.position.line + 1, lexer.current().toString().c_str(), extra ? extra : "");
      }
