...
#include "Luau-V3/Compiler/include/Luau/Compiler.h"
...
#include "Luau-V3/Compiler/include/Luau/BytecodeBuilder.h"
...
namespace Luau {
      ...
      struct Compiler {
            ...
            Compiler(BytecodeBuilder &bytecode, const CompileOptions &options)
                : bytecode(bytecode), options(options), functions(nullptr), locals(nullptr), globals(AstName()), variables(nullptr), constants(nullptr), locstants(nullptr), tableShapes(nullptr), builtins(nullptr) {
            ...
            void compileExprFastcallN(
                AstExprCall *expr, uint8_t target, uint8_t targetCount, bool targetTop, bool multRet, uint8_t regs, int bfid, int bfK = -1) {
                  ...
                  LuauOpcode opc = expr->args.size == 1 ? LOP_FASTCALL1 : (bfK >= 0 || isConstant(expr->args.data[1])) ? LOP_FASTCALL2K
                                                                                                                       : LOP_FASTCALL2;
            ...
            bool tryCompileInlinedCall(AstExprCall *expr, AstExprFunction *func, uint8_t target, uint8_t targetCount, bool multRet, int thresholdBase, int thresholdMaxBoost, int depthLimit) {
            ...
            void compileExprCall(AstExprCall *expr, uint8_t target, uint8_t targetCount, bool targetTop = false, bool multRet = false) {
                  ...
                  if (options.optimizationLevel >= 2 && !expr->self) {
                        ...
                        if (fi && fi->canInline &&
                            tryCompileInlinedCall(expr, func, target, targetCount, multRet, FInt::LuauCompileInlineThreshold, FInt::LuauCompileInlineThresholdMaxBoost, FInt::LuauCompileInlineDepth))
                              return;
